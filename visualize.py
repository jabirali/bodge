...
for i in lattice.sites():
    if diax(i, 1) < L_SC or diax(i, 1) >= L_X - L_SC:
        axs[0].scatter(x=i[0], y=i[1], color="#ff7f00")
    elif diax(i, 1) >= (L_SC + L_NM) and diax(i, 1) < L_X - (L_SC + L_NM):
        axs[0].scatter(x=i[0], y=i[1], color="#984ea3")
    else:
        axs[0].scatter(x=i[0], y=i[1], color="k")

for i in lattice.sites():
    if diax(i, 0) < L_SC or diax(i, 0) >= L_X - L_SC:
        axs[1].scatter(x=i[0], y=i[1], color="#ff7f00")
    elif diax(i, 0) >= L_SC + L_NM and diax(i, 0) < L_X - L_SC - L_NM:
