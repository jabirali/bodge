...
- It scales well to large systems. For efficiency, it uses SciPy sparse matrices internally [@scipy_2020], and it manages to construct large Hamiltonians in $\mathcal{O}(N)$ time and memory where $N$ is the number of sites. According to [my benchmarks](https://jabirali.github.io/bodge/tutorial.html#numerical-details), the performance is similar to [Kwant](https://kwant-project.org/) [@groth_kwant_2014], which is the state of the art for numerical condensed matter physics. The results can be returned in most useful NumPy or SciPy matrix formats.
- It is designed to be extensible. For instance, while Bodge currently only implements cubic lattices, it can be used to construct Hamiltonians on e.g. triangular or hexagonal lattices if you want: you just need to subclass `Lattice` and implement 2-3 short iterators for your new lattice.
